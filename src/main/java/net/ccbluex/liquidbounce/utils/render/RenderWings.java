...
public class RenderWings extends ModelBase {
    ...
    private ResourceLocation location = new ResourceLocation("liquidbounce/wings.png");
    ...
    public void renderWings(float partialTicks) {
        ...
        boolean per = this.mc.gameSettings.thirdPersonView == 0;
        double scale = 1.0D;
        float var9 = mc.thePlayer.prevRenderYawOffset;
        if(!per) {
            if (!LiquidBounce.moduleManager.getModule(KillAura.class).getState() || Aura.getTarget() == null) {
                if (!LiquidBounce.moduleManager.getModule(Scaffold.class).getState()) {
                    var9 = mc.thePlayer.prevRenderYawOffset;
                }
            }
        }
        ...
        if(!per) {
            if (!LiquidBounce.moduleManager.getModule(KillAura.class).getState() || Aura.getTarget() == null) {
                if (!LiquidBounce.moduleManager.getModule(Scaffold.class).getState()) {
                    var11 = mc.thePlayer.renderYawOffset;
                }
            }
        }

        double rotate = this.interpolate(var9, var11, partialTicks);
        GL11.glPushMatrix();
        GL11.glScaled(-scale, -scale, scale);
        GL11.glRotated(180.0D + rotate, 0.0D, 1.0D, 0.0D);
        boolean playerUsesFullHeight = true;
        GL11.glTranslated(0.0D, -1.45D / scale, 0.0D);
        GL11.glTranslated(0.0D, 0.0D, 0.2D / scale);
        if(mc.thePlayer.isSneaking()) {
            GL11.glTranslated(0.0D, 0.125D / scale, 0.0D);
        }
    ...
    private double interpolate(float yaw1, float yaw2, float percent) {
        double f = (double)(yaw1 + (yaw2 - yaw1) * percent) % 360.0D;
        if(f < 0.0D) {
            f += 360.0D;
        }
