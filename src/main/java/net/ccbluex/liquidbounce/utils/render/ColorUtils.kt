...
object ColorUtils {
    ...
    @JvmStatic
    fun CatRainbow(delay: Int,offset: Int, index: Int): Int {
    var rdelay = Math.ceil((System.currentTimeMillis() + (delay * index).toLong()).toDouble()) / offset
        return Color.getHSBColor(if ((360.0.also { rdelay %= it } / 60.0).toFloat().toDouble() < 0.5)
         - (rdelay/ 60.0).toFloat() else (rdelay / 30).toFloat(),0.5f,0.75f).rgb
    }
