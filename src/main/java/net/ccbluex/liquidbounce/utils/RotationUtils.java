...
public final class RotationUtils extends MinecraftInstance implements Listenable {
    ...
    public static Rotation getNovolineRotation(Entity entity) {
        if (entity == null) {
            return null;
        }
        double posX = entity.posX - mc.thePlayer.posX;
        double posY = entity.getEntityBoundingBox().minY + entity.getEyeHeight() * 0.9 - (mc.thePlayer.posY + mc.thePlayer.getEyeHeight());
        double posZ = entity.posZ - mc.thePlayer.posZ;
        double otherPos = MathHelper.ceiling_float_int((float) (posX * posX + posZ * posZ));
        float playerYaw = (float) (Math.atan2(posZ, posX) * 180.0F / Math.PI) - 90.0F;
        float playerPitch = (float)(-(Math.atan2(posY, otherPos) * 180.0 / Math.PI));
        return new Rotation(mc.thePlayer.rotationYaw + MathHelper.wrapAngleTo180_float(playerYaw - mc.thePlayer.rotationYaw), mc.thePlayer.rotationPitch + MathHelper.wrapAngleTo180_float(playerPitch - mc.thePlayer.rotationPitch));
    }
    /**
     *
     * @param entity
    ...
    public static VecRotation searchCenter2(final AxisAlignedBB bb, final boolean outborder, final boolean random, final boolean predict, final boolean throughWalls,final float maxrange) {
        if(outborder) {
