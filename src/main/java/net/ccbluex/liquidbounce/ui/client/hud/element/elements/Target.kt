...
class TargetHud : Element() {
    ...
    private val fontValue = FontValue("Font", Fonts.misans30)
    ...
    override fun drawElement(): Border? {
        ...
        if (target is EntityPlayer) {
            ...
            RenderUtils.drawGradientSideways(36.0, 16.0, (36.0F + (easingHealth / target.maxHealth) * (width - 36.0F)).toDouble(),
                24.0, Palette.fade2(customColor,counter1[0], fontRenderer.FONT_HEIGHT).rgb,
            ...
            fontRenderer.drawStringWithShadow(BigDecimal((target.health / target.maxHealth * 100).toDouble()).setScale(1, BigDecimal.ROUND_HALF_UP).toString() + "%", width / 2F + 6.5F, 17F, Color.white.rgb)
        }
