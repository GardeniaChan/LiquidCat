...
@ModuleInfo(name = "Aura", category = ModuleCategory.COMBAT, keyBind = Keyboard.KEY_R, description="kill youer mother")
class KillAura : Module() {
    ...
    private val rotationModeValue = ListValue("RotationMode", arrayOf("None","LiquidBounce","Hypixel","LockView", "OldMatrix","Full"), "LiquidBounce")
    // Bypass
    ...
    // AutoBlock
    val autoBlockValue = ListValue("AutoBlock", arrayOf("Range", "Fake","HytPit","Hypixel"),"Fake")
    ...
    private val autoBlockPacketValue = ListValue("AutoBlockPacket", arrayOf("AfterTick", "AfterAttack", "Vanilla","Off"),"AfterTick")
    ...
    private val switchDelayValue = IntegerValue("SwitchDelay",300 ,0, 2000)
    ...
    private fun attackEntity(entity: EntityLivingBase) {
        ...
        if(autoBlockValue.get().equals("Fake",true) && (mc.thePlayer.isBlocking || blockingStatus)){
            mc.netHandler.addToSendQueue(C07PacketPlayerDigging(C07PacketPlayerDigging.Action.RELEASE_USE_ITEM,BlockPos.ORIGIN,EnumFacing.DOWN))
            blockingStatus = false
        }
    ...
    private fun updateRotations(entity: Entity): Boolean {
        ...
        val rotation = when (rotationModeValue.get()) {
            ...
            "Full" -> RotationUtils.limitAngleChange(RotationUtils.serverRotation,directRotation,(Math.random() * (maxTurnSpeed.get() - minTurnSpeed.get()) + minTurnSpeed.get()).toFloat())
