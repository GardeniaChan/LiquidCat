...
//        val heldItem = mc.thePlayer.heldItem
        if (modeValue.get().toLowerCase() == "aac5") {
            if (event.eventState == EventState.POST && (mc.thePlayer.isUsingItem || mc.thePlayer.isBlocking)) {
        ...
        if (modeValue.get().toLowerCase() != "aac5") {
            ...
            when (modeValue.get().toLowerCase()) {
                ...
                "watchdog" -> {
                    if (sendPacketValue.get() && !mc.thePlayer.isBlocking) {
    ...
    fun onPacket(event: PacketEvent) {
        ...
        if (modeValue.get().equals("watchdog", true) && event.packet is S30PacketWindowItems && (mc.thePlayer.isUsingItem() || mc.thePlayer.isBlocking())) {
            ...
            if (debugValue.get())
                alert("Detected reset item packet","Cat")
        }
