...
class Criticals : Module() {
    ...
    val modeValue = ListValue("Mode", arrayOf("NewPacket", "Packet","Vulan", "NCPPacket", "NoGround", "Redesky", "AACv4", "Hop", "TPHop", "Jump", "Visual", "Edit", "MiniPhase", "NanoPacket", "Non-Calculable", "Invalid", "VerusSmart"), "Packet")
    ...
    fun onAttack(event: AttackEvent) {
        ...
        if (event.targetEntity is EntityLivingBase) {
            ...
            if (!mc.thePlayer.onGround || mc.thePlayer.isOnLadder || mc.thePlayer.isInWeb || mc.thePlayer.isInWater ||
                ...
                LiquidBounce.moduleManager[Flight::class.java]!!.state || !msTimer.hasTimePassed(delayValue.get().toLong()))
                return
            ...
            when (modeValue.get().toLowerCase()) {
                ...
                "vulan" -> {
                    ...
                    if (attacks > 6) {
                        mc.netHandler.addToSendQueue(C04PacketPlayerPosition(x,y + 0.2,z,false))
                        mc.netHandler.addToSendQueue(C04PacketPlayerPosition(x,y + 0.1216,z,false))
                        mc.thePlayer.onCriticalHit(entity)
                        attacks = 0
                    } else {
                        antiDesync = false
