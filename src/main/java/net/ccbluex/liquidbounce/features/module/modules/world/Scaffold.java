...
public class Scaffold extends Module {
    ...
    // Rotations
    public final ListValue rotationsValue = new ListValue("Rotations", new String[]{"None", "Custom","Custom2", "Vanilla", "AAC", "Hypixel"}, "AAC");
    ...
    private final IntegerValue custom2yaw = new IntegerValue("Custom2yaw",400,0,800);
    private final IntegerValue custom2pitch = new IntegerValue("Custom2Pitch",80,60,90);
    ...
    public void onUpdate(final UpdateEvent event) {
        ...
        if(sprintValue.getValue() && MovementUtils.isMoving()){
        ...
        if (shouldGoDown) launchY = (int) mc.thePlayer.posY - 1;
            ...
            if ((!autoJumpValue.get() && !(smartSpeedValue.get() && LiquidBounce.moduleManager.getModule(Speed.class).getState())) || GameSettings.isKeyDown(mc.gameSettings.keyBindJump) || mc.thePlayer.posY < launchY)
                launchY = (int) mc.thePlayer.posY;
            if (autoJumpValue.get() && !LiquidBounce.moduleManager.getModule(Speed.class).getState() && MovementUtils.isMoving() && mc.thePlayer.onGround) {
                mc.thePlayer.jump();
    ...
    private boolean search(final BlockPos blockPosition, final boolean checks) {
        ...
        if ((!rotationsValue.get().equals("None"))) {
            ...
            switch (rotationsValue.get().toLowerCase()) {
                ...
                case "custom2":{
                    rotation = new Rotation(custom2yaw.get(),custom2pitch.get());
                }
            }
            if (rotation != null) {
