...
class InventoryCleaner : Module() {
    ...
    private val disablerValue = BoolValue("AutoDisabler",true)
    ...
    @EventTarget
    private fun PacketEvent(event: PacketEvent) {
        if (disablerValue.get()) {
            val p = event.packet
            if (p is S02PacketChat) {
                val t = p.chatComponent.unformattedText
                if (t.contains("你现在是观察者状态. 按E打开菜单.", true)) {
                    LiquidBounce.hud.addNotification(Notification(name, "InventoryCleaner autodisable", NotifyType.WARNING))
                    state = false
                }
            }
        }
